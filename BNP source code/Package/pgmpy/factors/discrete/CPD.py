...
class TabularCPD(DiscreteFactor):
    ...
    def get_values(self):
        ...
        if self.variable in self.variables:
            #arcpy.AddMessage("parent:" + str(np.prod(self.cardinality[1:])))
            return self.values.reshape(self.cardinality[0], np.prod(self.cardinality[1:]))

        else:
            #arcpy.AddMessage("parent1:" + str(np.prod(self.cardinality)))
        ...
        if evidence:
            ...
            if self.state_names and print_state_names:
                for i in range(len(evidence_card)):
                    ...
                    header_list.append(['{var}({state})'.format
                                                     (var=evidence[i],
                                                      state=self.state_names[evidence[i]][d])
                                                     for d in col_indexes.T[i]])
            else:
                for i in range(len(evidence_card)):
                    ...
                    headers_list.append(column_header)
                    header_list.append(['{s}_{d}'.format(s=evidence[i], d=d) for d in col_indexes.T[i]])
...
#         return string
    def rtlstKey(self,lstA, lstB):
        lst = []
        cnt = len(lstB)
        arcpy.AddMessage('cnt:'+str(cnt))
        ...
        if cnt == 0:
            ...
            b = lstB[1]
            c = lstB[2]
            d = [a[i] + b[i] + c[i] for i in range(0, len(a))]
            for i in lstA[0]:
                for k in d:
                    lst.append(i + k)
        elif cnt == 4:
            ...
            c = lstB[2]
            d = lstB[3]
            e = [a[i] + b[i] + c[i] + d[i] for i in range(0, len(a))]
            for i in lstA[0]:
                for k in e:
                    lst.append(i + k)
        elif cnt == 5:
            ...
            d = lstB[3]
            e = lstB[4]
            f = [a[i] + b[i] + c[i] + d[i] + e[i] for i in range(0, len(a))]
            for i in lstA[0]:
                for k in f:
                    lst.append(i + k)
        elif cnt == 6:
            ...
            e = lstB[4]
            f = lstB[5]
            g = [a[i] + b[i] + c[i] + d[i] + e[i] + f[i] for i in range(0, len(a))]
            for i in lstA[0]:
                for k in g:
                    lst.append(i + k)
        elif cnt == 7:
            ...
            f = lstB[5]
            g = lstB[6]
            h = [a[i] + b[i] + c[i] + d[i] + e[i] + f[i] + g[i] for i in range(0, len(a))]
            for i in lstA[0]:
                for k in h:
                    lst.append(i + k)
        else:
            arcpy.AddMessage(str('error!!!!'))
        return lst
    def returndic(self):
        return self.dic